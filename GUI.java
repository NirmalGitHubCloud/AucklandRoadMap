...
public abstract class GUI {
	...
	/**
	 * Is called when the mouse is pressed and is passed
	 * the MouseEvent object for that press.
	 */	
	protected abstract void onPress(MouseEvent e);

	/**
	 * Is called when the mouse is dragged and is passed
	 * the MouseEvent object for that drag.
	 */	
	protected abstract void onDrag(MouseEvent e);
	
	/**
	 * Is called when the mouse wheel is turned and is passed
	 * the MouseWheelEvent object for that scroll.
	 */	
	 ...
	 * A* search
	 */
	protected abstract void aSearch(MouseEvent e1, MouseEvent e2, boolean time);
	...
	// fields needed for A* Search 
	private boolean findPath = false;
	private boolean time;
	private MouseEvent startNode = null;          // store clicked positions of start node
	private MouseEvent goalNode = null;           // store clicked positions of end node
	...
	private void initialise() {
		...
		JButton aSearchDist = new JButton("Find Path by Distance");
		aSearchDist.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				findPath = true;
				time = false;
			}
		...
		JButton aSearchTime = new JButton("Find Path by Time");
		aSearchTime.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				findPath = true;
				time = true;
			}
		...
		JButton artPoints = new JButton("Articulation Points");
		...
		// add JPanel for path finding searches
		JPanel searches = new JPanel();
		searches.setMaximumSize(new Dimension(150, 60));
		searches.setLayout(new GridLayout(2, 2));
		searches.add(aSearchDist);
		searches.add(aSearchTime);
		searches.add(artPoints);
		controls.add(searches);
		controls.add(Box.createRigidArea(new Dimension(15, 0)));
		...
		drawing.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent e) {
				if(findPath){
					if(startNode == null){
						startNode = e;
						aSearch(startNode, null, time);
					}
					else if(startNode != null && goalNode == null){
						goalNode = e;
						aSearch(null, goalNode, time);
						// redraw so goal node can be highlighted first
						redraw();
						//do A* search
						aSearch(startNode, goalNode, time);						
						//reset fields
						startNode = null;
						goalNode = null;
						findPath = false;
